...
public class Player extends Actor {
	...
	public Model getHeadModel() {
		...
		for (int index = 0; index < 5; index++)
			if (appearanceColors[index] != 0) {
				headModel.replaceColor(Game.playerColours[index][0],
						Game.playerColours[index][appearanceColors[index]]);
				if (index == 1)
		...
		if (model == null) {
			...
			for (int index = 0; index < 12; index++) {
				...
				if (part >= 256 && part < 512) {
					Model bodyModel = IdentityKit.cache[part - 256]
							.getBodyModel();
					if (bodyModel != null)
			...
			for (int part = 0; part < 5; part++)
				if (appearanceColors[part] != 0) {
					model.replaceColor(Game.playerColours[part][0],
							Game.playerColours[part][appearanceColors[part]]);
					if (part == 1)
	...
	public void updateAppearance(Buffer buffer) {
		...
		anInt1759 = buffer.getUnsignedShort();
		visible = true;
		appearanceHash = 0L;
		int k1 = appearance[5];
		int i2 = appearance[9];
